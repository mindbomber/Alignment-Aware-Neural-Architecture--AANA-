#!/usr/bin/env python
"""Launch the local AANA HTTP bridge with the web playground enabled."""
...
def build_parser():
    parser = argparse.ArgumentParser(description="Run the AANA adapter playground.")
    ...
    parser.add_argument(
        "--audit-log",
        default=str(ROOT / "eval_outputs" / "audit" / "playground" / "aana-playground.jsonl"),
        help="Path for redacted playground audit JSONL records.",
    )
    return parser
...
def main(argv=None):
    ...
    print(f"AANA playground: http://{args.host}:{args.port}/playground")
