...
def violation_constraint_ids(adapter, code):
    ...
    if "meeting_transcript_faithfulness_verified" in constraint_ids:
        return MEETING_SUMMARY_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "ticket_status_claims_verified" in constraint_ids:
        return TICKET_UPDATE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "answer_citations_index_verified" in constraint_ids:
        return RESEARCH_ANSWER_GROUNDING_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "incident_severity_verified" in constraint_ids:
        return INCIDENT_RESPONSE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "vulnerability_cve_facts_verified" in constraint_ids:
        return SECURITY_VULNERABILITY_DISCLOSURE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "access_requester_authority_verified" in constraint_ids:
        return ACCESS_PERMISSION_CHANGE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "migration_data_loss_reviewed" in constraint_ids:
        return DATABASE_MIGRATION_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "experiment_hypothesis_verified" in constraint_ids:
        return EXPERIMENT_LAUNCH_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "prd_acceptance_criteria_verified" in constraint_ids:
        return PRODUCT_REQUIREMENTS_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "vendor_contract_terms_reviewed" in constraint_ids:
        return PROCUREMENT_VENDOR_RISK_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "hiring_job_relatedness_verified" in constraint_ids:
        return HIRING_FEEDBACK_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "performance_review_evidence_supported" in constraint_ids:
        return PERFORMANCE_REVIEW_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "learning_curriculum_fit_verified" in constraint_ids:
        return LEARNING_TUTOR_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "api_breaking_changes_reviewed" in constraint_ids:
        return API_CONTRACT_CHANGE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "infra_blast_radius_reviewed" in constraint_ids:
        return INFRASTRUCTURE_CHANGE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "data_pipeline_schema_drift_reviewed" in constraint_ids:
        return DATA_PIPELINE_CHANGE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "model_eval_benchmark_claims_verified" in constraint_ids:
        return MODEL_EVALUATION_RELEASE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "feature_flag_audience_verified" in constraint_ids:
        return FEATURE_FLAG_ROLLOUT_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "sales_pricing_verified" in constraint_ids:
        return SALES_PROPOSAL_CHECKER_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "renewal_account_facts_verified" in constraint_ids:
        return CUSTOMER_SUCCESS_RENEWAL_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "billing_balance_facts_verified" in constraint_ids:
        return INVOICE_BILLING_REPLY_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "insurance_coverage_claims_verified" in constraint_ids:
        return INSURANCE_CLAIM_TRIAGE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "grant_eligibility_verified" in constraint_ids:
        return GRANT_APPLICATION_REVIEW_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "publication_claims_supported" in constraint_ids:
        return PUBLICATION_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "data_export_scope_verified" in constraint_ids:
        return DATA_EXPORT_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "calendar_availability_verified" in constraint_ids:
        return CALENDAR_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "live_quote_price_verified" in constraint_ids:
        return BOOKING_PURCHASE_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "investment_information_not_advice" in constraint_ids:
        return FINANCIAL_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "medical_information_not_advice" in constraint_ids:
        return MEDICAL_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "legal_jurisdiction_verified" in constraint_ids:
        return LEGAL_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "deployment_config_validated" in constraint_ids:
        return DEPLOYMENT_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "tests_and_ci_verified" in constraint_ids:
        return CODE_REVIEW_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "file_operation_scope_bound" in constraint_ids:
        return FILE_OPERATION_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "recipient_identity_verified" in constraint_ids:
        return EMAIL_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])
    if "crm_account_facts_verified" in constraint_ids:
        return CRM_VIOLATION_TO_CONSTRAINTS.get(code, ["unmapped_constraint"])

    mapped = VIOLATION_TO_CONSTRAINT.get(code)
...
def constraint_results(adapter, report):
    ...
    for violation in report.get("violations", []):
        for constraint_id in violation_constraint_ids(adapter, violation.get("code")):
...
Could you please review the project timeline when you have a chance?

No attachments will be included.

Before delivery, please explicitly confirm the recipient, message body, attachments, and send timing."""
