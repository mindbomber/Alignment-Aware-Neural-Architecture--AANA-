    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                f"```powershell",
                f"python scripts/validate_adapter.py --adapter {adapter_path.as_posix()}",
                "```",
                "",
                "Test the unsupported adapter path until domain-specific verifier logic is added:",
                "",
                "```powershell",
                f"python scripts/run_adapter.py --adapter {adapter_path.as_posix()} --prompt (Get-Content {prompt_path.as_posix()} -Raw) --candidate-file {candidate_path.as_posix()}",
                "```",
                "",
...
def main():
    ...
    print(json.dumps({"created": paths, "next_step": "Run scripts/validate_adapter.py on the generated adapter."}, indent=2))
