...
FALLBACK_CONFIG = {
    "schema_version": ADAPTER_GENERALIZATION_CONFIG_VERSION,
    "description": "Packaged fallback for runtime tool classification hints.",
    "tool_classification": {
        "read_policy_tools": ["calculate", "kb_search", "kb_lookup", "search_kb", "search_policy", "lookup_policy"],
        "private_read_hints": [
            "account",
            "booking",
            "cart",
        ...
        "risky_write_hints": ["delete", "deploy", "grant", "pay", "purchase", "reset", "revoke", "send", "transfer"],
        "public_read_hints": ["public", "score", "weather", "docs", "documentation", "search_web", "web_search", "status"],
        "risk_domain_keywords": {
            "devops": ["deploy", "ci", "kubernetes", "server", "secret", "env", "repo"],
            "finance": ["account", "card", "loan", "payment", "refund", "transfer", "invoice"],
            "hr": ["employee", "candidate", "payroll", "hr_"],
            "legal": ["legal", "contract", "case"],
            "pharma": ["patient", "medical", "drug", "trial", "pharma"],
            "public_information": ["public", "score", "weather", "docs", "documentation", "search_web", "web_search", "status"],
        },
    },
    "blocked_literal_policy": {
        "no_benchmark_ids": True,
        "no_answer_keys": True,
        "no_task_specific_probe_literals": True,
    },
}
...
def _cached_default_config(path: str) -> dict[str, Any]:
    config = load_adapter_generalization_config(path) if pathlib.Path(path).exists() else FALLBACK_CONFIG
