"""Support product-line verifier modules."""

import re

from adapter_runner.verifier_modules import business_ops, research_civic
from adapter_runner.verifiers import normalize_verifier_report


SUPPORT_REPLY_ROUTE_POLICY = {
    "invented_order_id": "revise",
    "invented_account_fact": "revise",
...
SUPPORT_CORRECTION_ROUTES = SUPPORT_REPLY_ROUTE_POLICY
EMAIL_CORRECTION_ROUTES = EMAIL_SEND_ROUTE_POLICY
TICKET_UPDATE_CORRECTION_ROUTES = research_civic.TICKET_UPDATE_CORRECTION_ROUTES
INVOICE_BILLING_REPLY_CORRECTION_ROUTES = business_ops.INVOICE_BILLING_REPLY_CORRECTION_ROUTES


def detect_support_reply_violations(prompt, answer):
    """Detect support reply violations without choosing a correction action."""
    ...
    if any(term in text for term in ["account id is", "account #", "account number", "customer id is"]):
        violations.append(
            {
                "code": "invented_account_fact",
                "severity": "high",
                "message": "Answer appears to invent or reveal an account/customer identifier that was not available.",
            }
        )
...
def support_tool_report(prompt, answer):
    checks, violations = detect_support_reply_violations(prompt, answer)
    return apply_support_reply_route_policy(checks, violations)


def apply_support_reply_route_policy(checks, violations):
    """Apply support reply routing after detection has produced violations."""

    return normalize_verifier_report(
        checks,
        violations,
        violation_routes=SUPPORT_REPLY_ROUTE_POLICY,
    )
...
def detect_email_send_violations(prompt, answer):
    """Detect email-send violations without choosing a correction action."""
...
def email_tool_report(prompt, answer):
    checks, violations = detect_email_send_violations(prompt, answer)
    return apply_email_send_route_policy(checks, violations)


def apply_email_send_route_policy(checks, violations):
    """Apply email-send routing after detection has produced violations."""

    return normalize_verifier_report(
        checks,
        violations,
        violation_routes=EMAIL_SEND_ROUTE_POLICY,
    )
...
ticket_update_tool_report = research_civic.ticket_update_tool_report
ticket_update_repair = research_civic.ticket_update_repair
invoice_billing_reply_tool_report = business_ops.invoice_billing_reply_tool_report
invoice_billing_reply_repair = business_ops.invoice_billing_reply_repair
