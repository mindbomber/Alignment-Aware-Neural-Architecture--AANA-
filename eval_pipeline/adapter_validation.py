...
def validate_adapter(adapter):
    ...
    if (adapter_name, domain_name) not in executable_adapters:
        add_issue(
            ...
            "warning",
            "runner",
            "This adapter can be validated, but scripts/adapters/run_adapter.py only has deterministic execution for the checked-in travel, meal-planning, support-reply, and research-summary adapters today.",
        )
...
def next_steps(errors):
    if errors:
        return [
            ...
            "Run scripts/validation/validate_adapter.py again after edits.",
        ]
    return [
        "Run this adapter with scripts/adapters/run_adapter.py.",
